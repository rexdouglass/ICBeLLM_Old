...
I think we should recut this to actions by civilians vs actions by armed. 

So 1 if 
## unarmed_vs_armed actor
...
unarmed - An action by unarmed actors. Think the kinds of policy decisions or interactions made by politicians and civilian leaders. Types of actions might include leadership change, coup, assassination, institutions change, protest or end protest, strike or end strike, riot or end riot, restrict rights or provide rights, terrorism or reduce terrorism, human rights violation or reduce human rights violation, mass killing or reduce mass killing, discussion, meeting, mediation, natural conclusion of diplomacy, sign formal agreement, settle dispute, join war on behalf of, formal military ally, mutual defense pact, economic cooperation, military cooperation, intelligence cooperation, unspecified cooperation, general political support, economic aid, humanitarian aid, military aid, unspecified aid, inspections, release captives, cede territory, allow access.

armed - An action by armed actors. Think the kinds of military or violent behaviors carried out by military units, armies, generals, rebel groups, etc. Types of actions might include raise in alert, mobilization, fortify, exercise, weapons test, deployment to area, show of force, blockade, border violation, no fly zone, battle/clash, attack, invasion/occupation, bombard, declaration of war, join ongoing war, continuation of previous fighting, assert political control over, annex, assert autonomy against.

End Codebook
"
valid_event <- classify_many_sentences(
                                       preamble=preamble,
                                       variables=c('event_type_armed_vs_unarmed'), 
                                       stratify_variables=c('event_type_armed_vs_unarmed'),
                                       keep_variables=c('event_type_armed_vs_unarmed'),
                                       training_constraints="sentence_type_includes_do %>% str_detect('^1')", #only train on classified do types
                                       predicting_constraints="sentence_type_includes_do %>% str_detect('^1')" #only predicting sentences with an act
                                       )
...
Begin Codebook
cooperative action by unarmed actors - unhelpful or aggresive actions by unarmed actors
uncooperative action by unarmed actors - helpful or peaceful actions by unarmed actors
escalatory action by armed actors - unhelpful or aggresive actions by armed actors
deescalatory action by armed actors - helpful or peaceful actions by armed actors
none - The sentence does not describe an action. It may describe a communication or a thought, but does not describe an actor performing an action.
End Codebook
"
valid_event <- classify_many_sentences(
                                       preamble=preamble,
